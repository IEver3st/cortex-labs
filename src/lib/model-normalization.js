...
export function fitGroundPlaneNormal(object, options = {}) {
  ...
  const maxLevelAngleRad = THREE.MathUtils.degToRad(
    options.maxLevelAngleDeg || DEFAULT_MAX_LEVEL_ANGLE_DEG,
  );
  const bandHeight = Math.max(
    size.y * (options.lowBandRatio || DEFAULT_LOW_BAND_RATIO),
    footprint * Math.sin(maxLevelAngleRad),
    maxDim * 0.01,
  );
  const sortedPoints = points.slice().sort((a, b) => a.y - b.y);
  let bandPointCount = 0;
  const cutoff = bounds.min.y + bandHeight;
  while (bandPointCount < sortedPoints.length && sortedPoints[bandPointCount].y <= cutoff) {
    bandPointCount += 1;
  }

  const minPointCount = Math.max(
    MIN_PLANE_POINTS,
    Math.round(sortedPoints.length * (options.lowPointFraction || DEFAULT_LOW_POINT_FRACTION)),
  );
  const maxPointCount = Math.max(
    MIN_PLANE_POINTS,
    Math.round(sortedPoints.length * (options.maxLowPointFraction || DEFAULT_MAX_LOW_POINT_FRACTION)),
  );
  const pointCount = Math.max(minPointCount, Math.min(Math.max(bandPointCount, MIN_PLANE_POINTS), maxPointCount));
  const lowPoints = sortedPoints.slice(0, pointCount);
