...
function processOccurrence(content, startIndex) {
    ...
    if (condVar) {
        let endTagIdx = firstPart.indexOf('</CyberButton>');
        let startTagCloseIdx = firstPart.lastIndexOf('>', endTagIdx - 1);

        let g1 = firstPart.slice(0, startTagCloseIdx + 1);
        let originalText = firstPart.slice(startTagCloseIdx + 1, endTagIdx).trim();
        let g3 = firstPart.slice(endTagIdx);
